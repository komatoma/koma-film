export type CategoryKey =
  | "animation"
  | "drama"
  | "romance"
  | "horror"
  | "action"
  | "sci-fi"
  | "war"
  | "comedy";

export interface Category {
  key: CategoryKey;
  label: string;
}

export const categories: Category[] = [
  { key: "animation", label: "მულტფილმები" },
  { key: "drama", label: "დრამა" },
  { key: "romance", label: "სიყვარული" },
  { key: "horror", label: "საშიში" },
  { key: "action", label: "ექშენი" },
  { key: "sci-fi", label: "ფანტასტიკა" },
  { key: "war", label: "ომი" },
  { key: "comedy", label: "კომედია" },
];

export const getCategoryLabel = (key: CategoryKey) =>
  categories.find((c) => c.key === key)?.label ?? key;

export interface Movie {
  id: string;
  title: string;
  year: number;
  rating: number;
  duration: string;
  category: CategoryKey;
  description: string;
  keywords: string[];
  image: string;
  /** YouTube video ID used for the embedded trailer */
  trailer: string;
}

const poster = (path: string) => `https://image.tmdb.org/t/p/w500${path}`;

export const movies: Movie[] = [
  // ─────────────── ფანტასტიკა (sci-fi) ───────────────
  {
    id: "inception",
    title: "Inception",
    year: 2010,
    rating: 8.8,
    duration: "2სთ 28წთ",
    category: "sci-fi",
    description:
      "გამოცდილი ქურდი, რომელიც სხვათა სიზმრებში იპარება, ბოლო დავალებას იღებს — იდეის დათესვას მიზნის გონებაში. რეალობასა და ილუზიას შორის საზღვარი ქრება და ის კარგავს ორიენტაციას საკუთარ ცნობიერებაში.",
    keywords: ["dream", "სიზმარი", "heist", "lost", "დაკარგვა", "sci-fi", "ფანტასტიკა", "thriller"],
    image: poster("/9gk7adHYeDvHkCSEqAvQNLV5Uge.jpg"),
    trailer: "YoHD9XEInc0",
  },
  {
    id: "interstellar",
    title: "Interstellar",
    year: 2014,
    rating: 8.7,
    duration: "2სთ 49წთ",
    category: "sci-fi",
    description:
      "როცა დედამიწა კვდება, მამა და ასტრონავტთა ჯგუფი ჭიის ხვრელში მოგზაურობენ კაცობრიობის გადასარჩენად. დაკარგული დროის და დაკარგული ოჯახის ისტორია სივრცისა და სიყვარულის ფონზე.",
    keywords: ["space", "კოსმოსი", "lost", "დაკარგვა", "sci-fi", "ფანტასტიკა", "family", "love"],
    image: poster("/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg"),
    trailer: "zSWdZVtXT7E",
  },
  {
    id: "the-matrix",
    title: "The Matrix",
    year: 1999,
    rating: 8.7,
    duration: "2სთ 16წთ",
    category: "sci-fi",
    description:
      "ჰაკერი აღმოაჩენს, რომ რეალობა, რომელშიც ცხოვრობს, მანქანების მიერ შექმნილი ილუზიაა. მას ურჩევნია ჭეშმარიტება, რაც არ უნდა მძიმე იყოს, და კაცობრიობის ბრძოლას უერთდება.",
    keywords: ["sci-fi", "ფანტასტიკა", "action", "ექშენი", "cyberpunk", "reality"],
    image: poster("/f89U3ADr1oiB1s9GkdPOEpXUk5H.jpg"),
    trailer: "vKQi3bBA1y8",
  },
  {
    id: "avatar",
    title: "Avatar",
    year: 2009,
    rating: 7.6,
    duration: "2სთ 42წთ",
    category: "sci-fi",
    description:
      "პარაპლეგიკი ჯარისკაცი პლანეტა პანდორაზე იგზავნება, სადაც ავატარის სხეულით ადგილობრივ ხალხს ერწყმის. მალე მას მათი სამყაროს დაცვა უწევს დამპყრობლებისგან.",
    keywords: ["sci-fi", "ფანტასტიკა", "adventure", "action", "ექშენი", "alien"],
    image: poster("/kyeqWdyUXW608qlYkRqosgbbJyK.jpg"),
    trailer: "5PSNL1qE6VY",
  },
  {
    id: "blade-runner-2049",
    title: "Blade Runner 2049",
    year: 2017,
    rating: 8.0,
    duration: "2სთ 44წთ",
    category: "sci-fi",
    description:
      "ახალგაზრდა ბლეიდ რანერი აღმოაჩენს დიდი ხნის დაფარულ საიდუმლოს, რომელსაც შეუძლია საზოგადოების ქაოსში ჩაძირვა. მისი ძიება მას დაკარგული გმირის კვალზე მიიყვანს.",
    keywords: ["sci-fi", "ფანტასტიკა", "noir", "future", "lost", "დაკარგვა"],
    image: poster("/gajva2L0rPYkEWjzgFlBXCAVBE5.jpg"),
    trailer: "gCcx85zbxz4",
  },
  {
    id: "dune",
    title: "Dune",
    year: 2021,
    rating: 8.0,
    duration: "2სთ 35წთ",
    category: "sci-fi",
    description:
      "ნიჭიერი ახალგაზრდა პოლ ატრეიდესი უდაბნო პლანეტა არაკისზე ხვდება, სადაც სამყაროს ყველაზე ძვირფასი რესურსი მოიპოვება. მას ბედი დიდი ბრძოლისკენ უბიძგებს.",
    keywords: ["sci-fi", "ფანტასტიკა", "epic", "desert", "adventure"],
    image: poster("/d5NXSklXo0qyIYkgV94XAgMIckC.jpg"),
    trailer: "8g18jFHCLXk",
  },
  {
    id: "arrival",
    title: "Arrival",
    year: 2016,
    rating: 7.9,
    duration: "1სთ 56წთ",
    category: "sci-fi",
    description:
      "როცა უცხოპლანეტელთა ხომალდები დედამიწაზე ჩნდება, ლინგვისტი ცდილობს მათ ენას გაუგოს. ამ პროცესში ის დროის აღქმას სრულიად ახლებურად აღიქვამს.",
    keywords: ["sci-fi", "ფანტასტიკა", "alien", "language", "drama"],
    image: poster("/x2FJsf1ElAgr63Y3PNPtJrcmpoe.jpg"),
    trailer: "tFMo3UJ4B4g",
  },

  // ─────────────── სიყვარული (romance) ───────────────
  {
    id: "titanic",
    title: "Titanic",
    year: 1997,
    rating: 7.9,
    duration: "3სთ 14წთ",
    category: "romance",
    description:
      "სხვადასხვა ფენის ორი ახალგაზრდა უიღბლო გემ ტიტანიკზე ერთმანეთს შეუყვარდებათ. სიყვარულის ისტორია, რომელიც კატასტროფად იქცევა ისტორიაში ცნობილ ღამეს.",
    keywords: ["love", "romance", "სიყვარული", "drama", "tragedy"],
    image: poster("/9xjZS2rlVxm8SFx8kPC3aIGCOYQ.jpg"),
    trailer: "kVrqfYjkTdQ",
  },
  {
    id: "la-la-land",
    title: "La La Land",
    year: 2016,
    rating: 8.0,
    duration: "2სთ 8წთ",
    category: "romance",
    description:
      "ჯაზ პიანისტი და მსახიობობის მოსურნე ქალი ლოს-ანჯელესში ერთმანეთს შეუყვარდებათ. სიყვარულის, ოცნებისა და მსხვერპლის მუსიკალური ისტორია.",
    keywords: ["love", "romance", "სიყვარული", "music", "musical", "drama"],
    image: poster("/uDO8zWDhfWwoFdKS4fzkUJt0Rf0.jpg"),
    trailer: "0pdqf4P9MB8",
  },
  {
    id: "the-notebook",
    title: "The Notebook",
    year: 2004,
    rating: 7.8,
    duration: "2სთ 3წთ",
    category: "romance",
    description:
      "ღარიბი ბიჭისა და მდიდარი გოგოს სიყვარულის ისტორია, რომელსაც დრო, ომი და საზოგადოება ცდილობს გათიშოს. მარადიული სიყვარულის შესახებ დაუვიწყარი მოთხრობა.",
    keywords: ["love", "romance", "სიყვარული", "drama"],
    image: poster("/qom1SZSENdmHFNZBXbtJAU0WTlC.jpg"),
    trailer: "FC6biTjEyZw",
  },
  {
    id: "eternal-sunshine",
    title: "Eternal Sunshine of the Spotless Mind",
    year: 2004,
    rating: 8.3,
    duration: "1სთ 48წთ",
    category: "romance",
    description:
      "წყვილი გადაწყვეტს, წაშალონ ერთმანეთის მოგონებები განშორების შემდეგ. მაგრამ პროცესის დროს კაცი ხვდება, რომ არ სურს დაკარგოს სიყვარული, რომელიც ერთხელ ჰქონდა.",
    keywords: ["love", "სიყვარული", "lost", "დაკარგვა", "sci-fi", "drama"],
    image: poster("/5MwkWH9tYHv3mV9OdYTMR5qreIz.jpg"),
    trailer: "07-QBnEkgXU",
  },
  {
    id: "pride-and-prejudice",
    title: "Pride & Prejudice",
    year: 2005,
    rating: 7.8,
    duration: "2სთ 9წთ",
    category: "romance",
    description:
      "ჭკვიანი და დამოუკიდებელი ელიზაბეტ ბენეტი და ამაყი მისტერ დარსი ერთმანეთს იჯერებენ სიამაყისა და წინასწარგანწყობის მიღმა. კლასიკური სიყვარულის ისტორია.",
    keywords: ["love", "romance", "სიყვარული", "drama", "classic"],
    image: poster("/sGjIvtVvTlWnia2zfJfHz81pZ9Q.jpg"),
    trailer: "cD2i-i0LNIM",
  },

  // ─────────────── მულტფილმები (animation) ───────────────
  {
    id: "toy-story",
    title: "Toy Story",
    year: 1995,
    rating: 8.3,
    duration: "1სთ 21წთ",
    category: "animation",
    description:
      "სათამაშოები ცოცხლდებიან, როცა ადამიანები არ უყურებენ. კოვბოი ვუდი და კოსმონავტი ბაზ ლაითიარი მეგობრდებიან თავგადასავლების შემდეგ.",
    keywords: ["animation", "მულტფილმი", "family", "adventure", "comedy"],
    image: poster("/uXDfjJbdP4ijW5hWSBrPrlKpxab.jpg"),
    trailer: "v-PjgYDrg70",
  },
  {
    id: "the-lion-king",
    title: "The Lion King",
    year: 1994,
    rating: 8.5,
    duration: "1სთ 28წთ",
    category: "animation",
    description:
      "ლომის ბელი სიმბა უნდა შეეჯახოს მამის დაკარგვის ტკივილს და დაიბრუნოს კუთვნილი სამეფო. ზრდის, პასუხისმგებლობისა და გამბედაობის ისტორია.",
    keywords: ["animation", "მულტფილმი", "family", "lost", "დაკარგვა", "adventure"],
    image: poster("/sKCr78MXSLixwmZ8DyJLrpMsd15.jpg"),
    trailer: "4sj1MT05lAA",
  },
  {
    id: "finding-nemo",
    title: "Finding Nemo",
    year: 2003,
    rating: 8.2,
    duration: "1სთ 40წთ",
    category: "animation",
    description:
      "მოშიშარა თევზი მარლინი მთელ ოკეანეს გადაივლის დაკარგული შვილის, ნემოს საპოვნელად. მამობრივი სიყვარულისა და გამბედაობის თბილი ისტორია.",
    keywords: ["animation", "მულტფილმი", "family", "lost", "დაკარგვა", "ocean"],
    image: poster("/eHuGQ10FUzK1mdOY69wF5pGgEf5.jpg"),
    trailer: "SPHfeNgogVs",
  },
  {
    id: "up",
    title: "Up",
    year: 2009,
    rating: 8.3,
    duration: "1სთ 36წთ",
    category: "animation",
    description:
      "მოხუცი კარლი ათასობით ბუშტს აბამს სახლზე და ცხოვრების ოცნებისკენ მიფრინავს. გზად ის მარტოობასა და დაკარგულ სიყვარულს პატარა მეგობართან ერთად ამარცხებს.",
    keywords: ["animation", "მულტფილმი", "family", "adventure", "love", "lost", "დაკარგვა"],
    image: poster("/mFvoEwSfLqbcWwFsDjQebn9bzFe.jpg"),
    trailer: "F2bk_9T482g",
  },
  {
    id: "frozen",
    title: "Frozen",
    year: 2013,
    rating: 7.4,
    duration: "1სთ 42წთ",
    category: "animation",
    description:
      "პრინცესა ანა უშიშარი გულით ეძებს დას ელზას, რომლის ჯადოსნურმა ძალამ სამეფო მარადიულ ზამთარში ჩააგდო. დობის სიყვარულის ისტორია.",
    keywords: ["animation", "მულტფილმი", "family", "musical", "love"],
    image: poster("/kgwjIb2JDHRhNk13lmSxiClFjVk.jpg"),
    trailer: "TbQm5doF_Uc",
  },
  {
    id: "coco",
    title: "Coco",
    year: 2017,
    rating: 8.4,
    duration: "1სთ 45წთ",
    category: "animation",
    description:
      "ბიჭი მიგელი მიცვალებულთა სამყაროში ხვდება ოჯახის საიდუმლოს გასაგებად და მუსიკის სიყვარულის დასაცავად. ოჯახის, მეხსიერებისა და მუსიკის ისტორია.",
    keywords: ["animation", "მულტფილმი", "family", "music", "love"],
    image: poster("/gGEsBPAijhVUFoiNpgZXqRVWJt2.jpg"),
    trailer: "Ga6RYejo6Hk",
  },
  {
    id: "shrek",
    title: "Shrek",
    year: 2001,
    rating: 7.9,
    duration: "1სთ 30წთ",
    category: "animation",
    description:
      "მარტოსული ჭაობის ურჩხული შრეკი მოგზაურობს პრინცესა ფიონას გამოსახსნელად. სახუმარო და გულთბილი ისტორია იმაზე, რომ სილამაზე შიგნიდან მოდის.",
    keywords: ["animation", "მულტფილმი", "comedy", "family", "love", "სიყვარული"],
    image: poster("/iB64vpL3dIObOtMZgX3RqdVdQDc.jpg"),
    trailer: "CwXOrWvPBPk",
  },
  {
    id: "spirited-away",
    title: "Spirited Away",
    year: 2001,
    rating: 8.5,
    duration: "2სთ 5წთ",
    category: "animation",
    description:
      "გოგონა ჩიჰირო მოხვდება სულების ჯადოსნურ სამყაროში, სადაც მშობლები ღორებად იქცნენ. მათი გადასარჩენად მას გამბედაობა და სიმტკიცე მართებს.",
    keywords: ["animation", "მულტფილმი", "anime", "fantasy", "adventure"],
    image: poster("/39wmItIWsg5sZMyRUHLkWBcuVCM.jpg"),
    trailer: "ByXuk9QqQkk",
  },

  // ─────────────── დრამა (drama) ───────────────
  {
    id: "the-shawshank-redemption",
    title: "The Shawshank Redemption",
    year: 1994,
    rating: 9.3,
    duration: "2სთ 22წთ",
    category: "drama",
    description:
      "უდანაშაულოდ გასამართლებული ბანკირი წლების განმავლობაში ინარჩუნებს იმედს ციხის კედლებში. მეგობრობის, თავისუფლებისა და შეურყევლობის ისტორია.",
    keywords: ["drama", "დრამა", "prison", "hope", "friendship"],
    image: poster("/q6y0Go1tsGEsmtFryDOJo3dEmqu.jpg"),
    trailer: "6hB3S9bIaco",
  },
  {
    id: "the-godfather",
    title: "The Godfather",
    year: 1972,
    rating: 9.2,
    duration: "2სთ 55წთ",
    category: "drama",
    description:
      "მაფიის კლანის დაბერებული ბელადი ძალაუფლებას ვაჟს გადასცემს. ოჯახის, ერთგულებისა და ძალაუფლების ეპიკური ისტორია.",
    keywords: ["drama", "დრამა", "crime", "family", "mafia"],
    image: poster("/3bhkrj58Vtu7enYsRolD1fZdja1.jpg"),
    trailer: "sY1S34973zA",
  },
  {
    id: "forrest-gump",
    title: "Forrest Gump",
    year: 1994,
    rating: 8.8,
    duration: "2სთ 22წთ",
    category: "drama",
    description:
      "მარტივი გულის მქონე კაცი უნებურად ხდება მე-20 საუკუნის უდიდესი მოვლენების მონაწილე — ომიდან სიყვარულამდე. ცხოვრების, დანაკარგისა და იმედის შთამბეჭდავი ისტორია.",
    keywords: ["love", "სიყვარული", "war", "ომი", "life", "drama", "დრამა"],
    image: poster("/arw2vcBveWOVZr6pxd9XTd1TdQa.jpg"),
    trailer: "bLvqoHBptjg",
  },
  {
    id: "cast-away",
    title: "Cast Away",
    year: 2000,
    rating: 7.8,
    duration: "2სთ 23წთ",
    category: "drama",
    description:
      "თვითმფრინავის კატასტროფის შემდეგ კაცი მარტო რჩება უკაცრიელ კუნძულზე. დაკარგული ცივილიზაციისგან, ის იბრძვის გადარჩენისთვის და საკუთარი თავის ხელახლა აღმოჩენისთვის.",
    keywords: ["lost", "დაკარგვა", "survival", "drama", "დრამა", "island"],
    image: poster("/7lLJgKnAicAcR5UEuo8xhSMj18w.jpg"),
    trailer: "qGuOZPwLayY",
  },
  {
    id: "fight-club",
    title: "Fight Club",
    year: 1999,
    rating: 8.4,
    duration: "2სთ 19წთ",
    category: "drama",
    description:
      "უძილობით დაქანცული ოფისის თანამშრომელი ჰარიზმატულ ტაილერ დერდენს ხვდება და მიწისქვეშა კლუბს ქმნიან. თვითდაკარგვისა და იდენტობის ბნელი ისტორია.",
    keywords: ["drama", "დრამა", "thriller", "lost", "დაკარგვა", "psychological"],
    image: poster("/pB8BM7pdSp6B6Ih7QZ4DrQ3PmJK.jpg"),
    trailer: "qtRKdVHc-cE",
  },
  {
    id: "the-green-mile",
    title: "The Green Mile",
    year: 1999,
    rating: 8.6,
    duration: "3სთ 9წთ",
    category: "drama",
    description:
      "სიკვდილმისჯილთა ბლოკის ზედამხედველი ხვდება პატიმარს, რომელსაც უჩვეულო, სასწაულებრივი ძალა აქვს. თანაგრძნობის, სამართლიანობისა და ადამიანურობის ისტორია.",
    keywords: ["drama", "დრამა", "prison", "supernatural", "emotional"],
    image: poster("/velWPhVMQeQKcxggNEU8YmIo52R.jpg"),
    trailer: "Ki4haFrqSrw",
  },
  {
    id: "joker",
    title: "Joker",
    year: 2019,
    rating: 8.4,
    duration: "2სთ 2წთ",
    category: "drama",
    description:
      "უგულებელყოფილი კომიკოსი არტურ ფლეკი ნელ-ნელა ეშვება სიგიჟისა და ძალადობის უფსკრულში. საზოგადოებისგან მიტოვებული ადამიანის ბნელი ფსიქოლოგიური პორტრეტი.",
    keywords: ["drama", "დრამა", "thriller", "crime", "psychological"],
    image: poster("/udDclJoHjfjb8Ekgsd4FDteOkCU.jpg"),
    trailer: "zAGVQLHvwOY",
  },

  // ─────────────── საშიში (horror) ───────────────
  {
    id: "the-conjuring",
    title: "The Conjuring",
    year: 2013,
    rating: 7.5,
    duration: "1სთ 52წთ",
    category: "horror",
    description:
      "პარანორმალურ მოვლენათა მკვლევრები ოჯახს ეხმარებიან, რომელსაც ბოროტი სული ტანჯავს განდეგილ ფერმაში. რეალურ მოვლენებზე დაფუძნებული საშინელებათა ისტორია.",
    keywords: ["horror", "საშიში", "supernatural", "thriller", "scary"],
    image: poster("/wVYREutTvI2tmxr6ujrHT704wGF.jpg"),
    trailer: "k10ETZ41q5o",
  },
  {
    id: "it",
    title: "It",
    year: 2017,
    rating: 7.3,
    duration: "2სთ 15წთ",
    category: "horror",
    description:
      "ბავშვთა ჯგუფი დაუპირისპირდება ბოროტ არსებას, რომელიც კლოუნ პენივაიზის სახით მათ შიშებს იკვებება. მეგობრობისა და გამბედაობის საშინელებათა ისტორია.",
    keywords: ["horror", "საშიში", "clown", "thriller", "scary"],
    image: poster("/9E2y5Q7WlCVNEhP5GiVTjhEhx1o.jpg"),
    trailer: "FnCdOQsX5kc",
  },
  {
    id: "the-shining",
    title: "The Shining",
    year: 1980,
    rating: 8.2,
    duration: "2სთ 26წთ",
    category: "horror",
    description:
      "მწერალი ოჯახთან ერთად ზამთრის სეზონზე მიტოვებულ სასტუმროში მუშაობს. იზოლაცია და ბოროტი ძალები მას ნელ-ნელა სიგიჟისკენ უბიძგებს.",
    keywords: ["horror", "საშიში", "psychological", "thriller", "classic"],
    image: poster("/b6ko0IKC8MdYBBPkkA1aBPLe2yz.jpg"),
    trailer: "S014oGZiSdI",
  },
  {
    id: "a-quiet-place",
    title: "A Quiet Place",
    year: 2018,
    rating: 7.5,
    duration: "1სთ 30წთ",
    category: "horror",
    description:
      "ოჯახი სრულ სიჩუმეში ცხოვრობს, რადგან ხმაზე მონადირე ბრმა არსებები მათ ემუქრებიან. დაძაბულობითა და მშობლური სიყვარულით სავსე საშინელებათა ისტორია.",
    keywords: ["horror", "საშიში", "thriller", "family", "survival"],
    image: poster("/nAU74GmpUk7t5iklEp3bufwDq4n.jpg"),
    trailer: "WR7cc5t7tv8",
  },
  {
    id: "get-out",
    title: "Get Out",
    year: 2017,
    rating: 7.8,
    duration: "1სთ 44წთ",
    category: "horror",
    description:
      "ახალგაზრდა კაცი შეყვარებულის ოჯახს სტუმრობს და თანდათან საშინელ საიდუმლოს აღმოაჩენს. სოციალური თრილერი ბნელი ატმოსფეროთი.",
    keywords: ["horror", "საშიში", "thriller", "mystery", "psychological"],
    image: poster("/tFXcEccSQMf3lfhfXKSU9iRBpa3.jpg"),
    trailer: "DzfpyUB60YY",
  },

  // ─────────────── ექშენი (action) ───────────────
  {
    id: "the-dark-knight",
    title: "The Dark Knight",
    year: 2008,
    rating: 9.0,
    duration: "2სთ 32წთ",
    category: "action",
    description:
      "ბეტმენი ქაოსში ჩაძირულ გოთემს იცავს ჯოკერისგან — გენიოსი კრიმინალისგან, რომელსაც ქალაქის სულის მოსპობა სურს. სიკეთისა და ბოროტების ეპიკური დაპირისპირება.",
    keywords: ["action", "ექშენი", "crime", "superhero", "thriller"],
    image: poster("/qJ2tW6WMUDux911r6m7haRef0WH.jpg"),
    trailer: "EXeTwQWrcwY",
  },
  {
    id: "gladiator",
    title: "Gladiator",
    year: 2000,
    rating: 8.2,
    duration: "2სთ 35წთ",
    category: "action",
    description:
      "მოღალატე იმპერატორის მიერ ღალატში ჩავარდნილი რომაელი გენერალი გლადიატორად იქცევა და შურისძიებას ესწრაფვის. ღირსებისა და გამბედაობის ეპოსი.",
    keywords: ["action", "ექშენი", "history", "epic", "revenge"],
    image: poster("/ty8TGRuvJLPUmAR1H1nRIsgwvim.jpg"),
    trailer: "owK1qxDselE",
  },
  {
    id: "mad-max-fury-road",
    title: "Mad Max: Fury Road",
    year: 2015,
    rating: 8.1,
    duration: "2სთ 0წთ",
    category: "action",
    description:
      "პოსტ-აპოკალიფსურ უდაბნოში მაქსი და მეამბოხე ფურიოსა ტირანს გაურბიან საშიშ დევნაში. დაუსრულებელი ადრენალინითა და მოძრაობით სავსე ექშენი.",
    keywords: ["action", "ექშენი", "post-apocalyptic", "chase", "survival"],
    image: poster("/hA2ple9q4qnwxp3hKVNhroipsir.jpg"),
    trailer: "hEJnMQG9ev8",
  },
  {
    id: "john-wick",
    title: "John Wick",
    year: 2014,
    rating: 7.4,
    duration: "1სთ 41წთ",
    category: "action",
    description:
      "პენსიაზე გასული კილერი შურისძიების გზას უბრუნდება, როცა კრიმინალები მას ყველაზე ძვირფასს ართმევენ. სტილური და დაუნდობელი ექშენი.",
    keywords: ["action", "ექშენი", "revenge", "thriller", "crime"],
    image: poster("/fZPSd91yGE9fCcCe6OoQr6E3Bev.jpg"),
    trailer: "C0BMx-qxsP4",
  },

  // ─────────────── ომი (war) ───────────────
  {
    id: "saving-private-ryan",
    title: "Saving Private Ryan",
    year: 1998,
    rating: 8.6,
    duration: "2სთ 49წთ",
    category: "war",
    description:
      "მეორე მსოფლიო ომის დროს ჯარისკაცთა ჯგუფი მიემართება მტრის ხაზების მიღმა, რათა იპოვონ და სახლში დააბრუნონ ერთი ჯარისკაცი. ომის სისასტიკისა და ძმობის ძლიერი ისტორია.",
    keywords: ["war", "ომი", "history", "drama", "military"],
    image: poster("/1wY4psJ5NVEhCuOYROwLH2XExM2.jpg"),
    trailer: "zwhP5b4tD6g",
  },
  {
    id: "1917",
    title: "1917",
    year: 2019,
    rating: 8.2,
    duration: "1სთ 59წთ",
    category: "war",
    description:
      "პირველი მსოფლიო ომის დროს ორ ახალგაზრდა ჯარისკაცს ავალებენ საშიშ მისიას — გადასცენ შეტყობინება, რომელიც ათასობით სიცოცხლეს გადაარჩენს. ერთი დაუსრულებელი კადრით გადაღებული ომის ეპოსი.",
    keywords: ["war", "ომი", "history", "drama", "military"],
    image: poster("/iZf0KyrE25z1sage4SYFLCCrMi9.jpg"),
    trailer: "YqNYrYUiMfg",
  },

  // ─────────────── კომედია (comedy) ───────────────
  {
    id: "the-grand-budapest-hotel",
    title: "The Grand Budapest Hotel",
    year: 2014,
    rating: 8.1,
    duration: "1სთ 39წთ",
    category: "comedy",
    description:
      "ლეგენდარული სასტუმროს კონსიერჟი და მისი ახალგაზრდა შეგირდი ძვირფასი ნახატის ქურდობასა და ოჯახური მემკვიდრეობის ბრძოლაში ეხვევიან. ფერადოვანი და ესთეტიკური კომედია.",
    keywords: ["comedy", "კომედია", "adventure", "quirky", "drama"],
    image: poster("/eWdyYQreja6JGCzqHWXpWHDrrPo.jpg"),
    trailer: "1Fg5iWmQjwk",
  },

  // ─────────────── დამატებული ფილმები (extended library) ───────────────
  {
    id: "the-martian",
    title: "The Martian",
    year: 2015,
    rating: 8.0,
    duration: "2სთ 24წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — The Martian.",
    keywords: ["sci-fi", "ფანტასტიკა", "space", "survival", "lost", "დაკარგვა"],
    image: poster("/AudsfLNJRLm2sfLtQXQjxdWmDkw.jpg"),
    trailer: "ej3ioOneTy8",
  },
  {
    id: "gravity",
    title: "Gravity",
    year: 2013,
    rating: 7.7,
    duration: "1სთ 31წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Gravity.",
    keywords: ["sci-fi", "ფანტასტიკა", "space", "survival"],
    image: poster("/2gCkbfMYjJqfmFsQtQ8g8w6vGWG.jpg"),
    trailer: "OiTiKOy59o4",
  },
  {
    id: "ex-machina",
    title: "Ex Machina",
    year: 2014,
    rating: 7.7,
    duration: "1სთ 48წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Ex Machina.",
    keywords: ["sci-fi", "ფანტასტიკა", "ai", "robot"],
    image: poster("/dmJW8IAKHKxFNiUnoDR7JfsK7Rp.jpg"),
    trailer: "XYGzRB4Pnq8",
  },
  {
    id: "edge-of-tomorrow",
    title: "Edge of Tomorrow",
    year: 2014,
    rating: 7.9,
    duration: "1სთ 53წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Edge of Tomorrow.",
    keywords: ["sci-fi", "ფანტასტიკა", "time", "alien"],
    image: poster("/uAKUyLbaAykZFYtrGGL6HdVIrJl.jpg"),
    trailer: "yUmSVcttXnI",
  },
  {
    id: "minority-report",
    title: "Minority Report",
    year: 2002,
    rating: 7.6,
    duration: "2სთ 25წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Minority Report.",
    keywords: ["sci-fi", "ფანტასტიკა", "future", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/44/Minority_Report_Poster.jpg",
    trailer: "B12Qc8Cr91E",
  },
  {
    id: "wall-e",
    title: "WALL-E",
    year: 2008,
    rating: 8.4,
    duration: "1სთ 38წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — WALL-E.",
    keywords: ["sci-fi", "ფანტასტიკა", "animation", "robot", "love"],
    image: poster("/hbhFnRzzg6ZDmm8YAmxBnQpQIPh.jpg"),
    trailer: "alIq_wG9FNk",
  },
  {
    id: "ready-player-one",
    title: "Ready Player One",
    year: 2018,
    rating: 7.4,
    duration: "2სთ 20წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Ready Player One.",
    keywords: ["sci-fi", "ფანტასტიკა", "virtual", "adventure"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/74/Ready_Player_One_%28film%29.png",
    trailer: "cSp1dM2Vj48",
  },
  {
    id: "looper",
    title: "Looper",
    year: 2012,
    rating: 7.4,
    duration: "1სთ 59წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Looper.",
    keywords: ["sci-fi", "ფანტასტიკა", "time", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/0a/Looper_poster.jpg",
    trailer: "2iQuhsmtfHw",
  },
  {
    id: "the-fifth-element",
    title: "The Fifth Element",
    year: 1997,
    rating: 7.6,
    duration: "2სთ 6წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — The Fifth Element.",
    keywords: ["sci-fi", "ფანტასტიკა", "adventure", "alien"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/65/Fifth_element_poster_%281997%29.jpg",
    trailer: "6JnN1DmbqoU",
  },
  {
    id: "star-wars-a-new-hope",
    title: "Star Wars: A New Hope",
    year: 1977,
    rating: 8.6,
    duration: "2სთ 1წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Star Wars: A New Hope.",
    keywords: ["sci-fi", "ფანტასტიკა", "space", "adventure", "epic"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/87/StarWarsMoviePoster1977.jpg",
    trailer: "vZ734NWnAHA",
  },
  {
    id: "the-empire-strikes-back",
    title: "The Empire Strikes Back",
    year: 1980,
    rating: 8.7,
    duration: "2სთ 4წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — The Empire Strikes Back.",
    keywords: ["sci-fi", "ფანტასტიკა", "space", "adventure", "epic"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/3f/The_Empire_Strikes_Back_%281980_film%29.jpg",
    trailer: "JNwNXF9Y6kY",
  },
  {
    id: "guardians-of-the-galaxy",
    title: "Guardians of the Galaxy",
    year: 2014,
    rating: 8.0,
    duration: "2სთ 1წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Guardians of the Galaxy.",
    keywords: ["sci-fi", "ფანტასტიკა", "space", "action", "comedy"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/33/Guardians_of_the_Galaxy_%28film%29_poster.jpg",
    trailer: "d96cjJhvlMA",
  },
  {
    id: "tenet",
    title: "Tenet",
    year: 2020,
    rating: 7.3,
    duration: "2სთ 30წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Tenet.",
    keywords: ["sci-fi", "ფანტასტიკა", "time", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/1/14/Tenet_movie_poster.jpg",
    trailer: "LdOM0x0XDMo",
  },
  {
    id: "district-9",
    title: "District 9",
    year: 2009,
    rating: 7.9,
    duration: "1სთ 52წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — District 9.",
    keywords: ["sci-fi", "ფანტასტიკა", "alien", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d7/District_nine_ver2.jpg",
    trailer: "DyLUwOcR5pk",
  },
  {
    id: "her",
    title: "Her",
    year: 2013,
    rating: 8.0,
    duration: "2სთ 6წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Her.",
    keywords: ["sci-fi", "ფანტასტიკა", "ai", "love", "romance", "სიყვარული"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/44/Her2013Poster.jpg",
    trailer: "dJTU48_yghs",
  },
  {
    id: "i-am-legend",
    title: "I Am Legend",
    year: 2007,
    rating: 7.2,
    duration: "1სთ 41წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — I Am Legend.",
    keywords: ["sci-fi", "ფანტასტიკა", "survival", "apocalypse", "lost", "დაკარგვა"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/df/I_am_legend_teaser.jpg",
    trailer: "dCA7NCRQTQU",
  },
  {
    id: "oblivion",
    title: "Oblivion",
    year: 2013,
    rating: 7.0,
    duration: "2სთ 4წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Oblivion.",
    keywords: ["sci-fi", "ფანტასტიკა", "future", "action"],
    image: "https://upload.wikimedia.org/wikipedia/en/2/2e/Oblivion2013Poster.jpg",
    trailer: "VntCPHYY8yw",
  },
  {
    id: "the-terminator",
    title: "The Terminator",
    year: 1984,
    rating: 8.1,
    duration: "1სთ 47წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — The Terminator.",
    keywords: ["sci-fi", "ფანტასტიკა", "robot", "action"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/6d/The_Terminator.png",
    trailer: "k64P4l2Wmeg",
  },
  {
    id: "terminator-2",
    title: "Terminator 2: Judgment Day",
    year: 1991,
    rating: 8.6,
    duration: "2სთ 17წთ",
    category: "sci-fi",
    description:
      "მასშტაბური სამეცნიერო-ფანტასტიკური მოგზაურობა მომავლისა და უცნობისკენ. — Terminator 2: Judgment Day.",
    keywords: ["sci-fi", "ფანტასტიკა", "robot", "action"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/5e/Terminator_2-Judgment_Day.png",
    trailer: "CRRlbK5w8AE",
  },
  {
    id: "inglourious-basterds",
    title: "Inglourious Basterds",
    year: 2009,
    rating: 8.3,
    duration: "2სთ 33წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Inglourious Basterds.",
    keywords: ["action", "ექშენი", "war", "ომი", "revenge"],
    image: poster("/7sfbEnaARXDDhKm0CZ7D7uc2sbo.jpg"),
    trailer: "KnrRy6kSFF0",
  },
  {
    id: "die-hard",
    title: "Die Hard",
    year: 1988,
    rating: 8.2,
    duration: "2სთ 12წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Die Hard.",
    keywords: ["action", "ექშენი", "thriller", "christmas"],
    image: "https://upload.wikimedia.org/wikipedia/en/c/ca/Die_Hard_%281988_film%29_poster.jpg",
    trailer: "jaJuwKCmJ78",
  },
  {
    id: "the-matrix-reloaded",
    title: "The Matrix Reloaded",
    year: 2003,
    rating: 7.2,
    duration: "2სთ 18წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — The Matrix Reloaded.",
    keywords: ["action", "ექშენი", "sci-fi", "ფანტასტიკა"],
    image: "https://upload.wikimedia.org/wikipedia/en/b/ba/Poster_-_The_Matrix_Reloaded.jpg",
    trailer: "kYzz0FSgpSU",
  },
  {
    id: "mission-impossible-fallout",
    title: "Mission: Impossible - Fallout",
    year: 2018,
    rating: 7.7,
    duration: "2სთ 27წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Mission: Impossible - Fallout.",
    keywords: ["action", "ექშენი", "spy", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/f/ff/MI_%E2%80%93_Fallout.jpg",
    trailer: "wb49-oV0F78",
  },
  {
    id: "skyfall",
    title: "Skyfall",
    year: 2012,
    rating: 7.8,
    duration: "2სთ 23წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Skyfall.",
    keywords: ["action", "ექშენი", "spy", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/a7/Skyfall_poster.jpg",
    trailer: "6kw1UVovByw",
  },
  {
    id: "casino-royale",
    title: "Casino Royale",
    year: 2006,
    rating: 8.0,
    duration: "2სთ 24წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Casino Royale.",
    keywords: ["action", "ექშენი", "spy", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/82/Casino_Royale_%282006_film_poster%29.jpg",
    trailer: "36mnx8dBbGE",
  },
  {
    id: "the-avengers",
    title: "The Avengers",
    year: 2012,
    rating: 8.0,
    duration: "2სთ 23წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — The Avengers.",
    keywords: ["action", "ექშენი", "superhero", "sci-fi"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8a/The_Avengers_%282012_film%29_poster.jpg",
    trailer: "eOrNdBpGMv8",
  },
  {
    id: "avengers-endgame",
    title: "Avengers: Endgame",
    year: 2019,
    rating: 8.4,
    duration: "3სთ 1წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Avengers: Endgame.",
    keywords: ["action", "ექშენი", "superhero", "epic"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/0d/Avengers_Endgame_poster.jpg",
    trailer: "TcMBFSGVi1c",
  },
  {
    id: "avengers-infinity-war",
    title: "Avengers: Infinity War",
    year: 2018,
    rating: 8.4,
    duration: "2სთ 29წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Avengers: Infinity War.",
    keywords: ["action", "ექშენი", "superhero", "epic"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/4d/Avengers_Infinity_War_poster.jpg",
    trailer: "6ZfuNTqbHE8",
  },
  {
    id: "iron-man",
    title: "Iron Man",
    year: 2008,
    rating: 7.9,
    duration: "2სთ 6წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Iron Man.",
    keywords: ["action", "ექშენი", "superhero"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/02/Iron_Man_%282008_film%29_poster.jpg",
    trailer: "8ugaeA-nMTc",
  },
  {
    id: "spider-man-no-way-home",
    title: "Spider-Man: No Way Home",
    year: 2021,
    rating: 8.2,
    duration: "2სთ 28წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Spider-Man: No Way Home.",
    keywords: ["action", "ექშენი", "superhero"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/00/Spider-Man_No_Way_Home_poster.jpg",
    trailer: "JfVOs4VSpmA",
  },
  {
    id: "black-panther",
    title: "Black Panther",
    year: 2018,
    rating: 7.3,
    duration: "2სთ 14წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Black Panther.",
    keywords: ["action", "ექშენი", "superhero"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d6/Black_Panther_%28film%29_poster.jpg",
    trailer: "xjDjIWPwcPU",
  },
  {
    id: "top-gun-maverick",
    title: "Top Gun: Maverick",
    year: 2022,
    rating: 8.2,
    duration: "2სთ 11წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Top Gun: Maverick.",
    keywords: ["action", "ექშენი", "military", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/1/13/Top_Gun_Maverick_Poster.jpg",
    trailer: "qSqVVswa420",
  },
  {
    id: "john-wick-chapter-4",
    title: "John Wick: Chapter 4",
    year: 2023,
    rating: 7.7,
    duration: "2სთ 49წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — John Wick: Chapter 4.",
    keywords: ["action", "ექშენი", "revenge", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d0/John_Wick_-_Chapter_4_promotional_poster.jpg",
    trailer: "qEVUtrk8_B4",
  },
  {
    id: "kingsman",
    title: "Kingsman: The Secret Service",
    year: 2014,
    rating: 7.7,
    duration: "2სთ 9წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Kingsman: The Secret Service.",
    keywords: ["action", "ექშენი", "spy", "comedy"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8b/Kingsman_The_Secret_Service_poster.jpg",
    trailer: "kDA01hZ399k",
  },
  {
    id: "baby-driver",
    title: "Baby Driver",
    year: 2017,
    rating: 7.5,
    duration: "1სთ 53წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Baby Driver.",
    keywords: ["action", "ექშენი", "heist", "chase"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8e/Baby_Driver_poster.jpg",
    trailer: "z2z857RSfhk",
  },
  {
    id: "the-bourne-identity",
    title: "The Bourne Identity",
    year: 2002,
    rating: 7.9,
    duration: "1სთ 59წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — The Bourne Identity.",
    keywords: ["action", "ექშენი", "spy", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/ae/BourneIdentityfilm.jpg",
    trailer: "tuIXWmDh64k",
  },
  {
    id: "300",
    title: "300",
    year: 2006,
    rating: 7.6,
    duration: "1სთ 57წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — 300.",
    keywords: ["action", "ექშენი", "war", "ომი", "epic", "history"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/5c/300poster.jpg",
    trailer: "W3zDvsyBd6M",
  },
  {
    id: "the-batman",
    title: "The Batman",
    year: 2022,
    rating: 7.8,
    duration: "2სთ 56წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — The Batman.",
    keywords: ["action", "ექშენი", "superhero", "crime", "noir"],
    image: "https://upload.wikimedia.org/wikipedia/en/f/ff/The_Batman_%28film%29_poster.jpg",
    trailer: "mqqft2x_Aa4",
  },
  {
    id: "logan",
    title: "Logan",
    year: 2017,
    rating: 8.1,
    duration: "2სთ 17წთ",
    category: "action",
    description:
      "ადრენალინით სავსე ექშენი განუწყვეტელი მოქმედებითა და დაძაბულობით. — Logan.",
    keywords: ["action", "ექშენი", "superhero", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/37/Logan_2017_poster.jpg",
    trailer: "Div0iP65aZo",
  },
  {
    id: "goodfellas",
    title: "Goodfellas",
    year: 1990,
    rating: 8.7,
    duration: "2სთ 25წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Goodfellas.",
    keywords: ["drama", "დრამა", "crime", "mafia"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/7b/Goodfellas.jpg",
    trailer: "2ilzidi_J8Q",
  },
  {
    id: "pulp-fiction",
    title: "Pulp Fiction",
    year: 1994,
    rating: 8.9,
    duration: "2სთ 34წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Pulp Fiction.",
    keywords: ["drama", "დრამა", "crime", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/3b/Pulp_Fiction_%281994%29_poster.jpg",
    trailer: "s7EdQ4FqbhY",
  },
  {
    id: "schindlers-list",
    title: "Schindler's List",
    year: 1993,
    rating: 9.0,
    duration: "3სთ 15წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Schindler's List.",
    keywords: ["drama", "დრამა", "war", "ომი", "history"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/38/Schindler%27s_List_movie.jpg",
    trailer: "gG22XNhtnoY",
  },
  {
    id: "the-godfather-part-ii",
    title: "The Godfather Part II",
    year: 1974,
    rating: 9.0,
    duration: "3სთ 22წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — The Godfather Part II.",
    keywords: ["drama", "დრამა", "crime", "family", "mafia"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/03/Godfather_part_ii.jpg",
    trailer: "9O1Iy9od7-A",
  },
  {
    id: "whiplash",
    title: "Whiplash",
    year: 2014,
    rating: 8.5,
    duration: "1სთ 46წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Whiplash.",
    keywords: ["drama", "დრამა", "music"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/01/Whiplash_poster.jpg",
    trailer: "7d_jQycdQGo",
  },
  {
    id: "the-departed",
    title: "The Departed",
    year: 2006,
    rating: 8.5,
    duration: "2სთ 31წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — The Departed.",
    keywords: ["drama", "დრამა", "crime", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/50/Departed234.jpg",
    trailer: "auYbpnEHBho",
  },
  {
    id: "parasite",
    title: "Parasite",
    year: 2019,
    rating: 8.5,
    duration: "2სთ 12წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Parasite.",
    keywords: ["drama", "დრამა", "thriller", "class"],
    image: poster("/7IiTTgloJzvGI1TAYymCfbfl3vT.jpg"),
    trailer: "5xH0HfJHsaY",
  },
  {
    id: "12-years-a-slave",
    title: "12 Years a Slave",
    year: 2013,
    rating: 8.1,
    duration: "2სთ 14წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — 12 Years a Slave.",
    keywords: ["drama", "დრამა", "history"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/5c/12_Years_a_Slave_film_poster.jpg",
    trailer: "z02Ie8wKKRg",
  },
  {
    id: "a-beautiful-mind",
    title: "A Beautiful Mind",
    year: 2001,
    rating: 8.2,
    duration: "2სთ 15წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — A Beautiful Mind.",
    keywords: ["drama", "დრამა", "biography"],
    image: "https://upload.wikimedia.org/wikipedia/en/b/b8/A_Beautiful_Mind_Poster.jpg",
    trailer: "aS_d0Ayjw4o",
  },
  {
    id: "the-pursuit-of-happyness",
    title: "The Pursuit of Happyness",
    year: 2006,
    rating: 8.0,
    duration: "1სთ 57წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — The Pursuit of Happyness.",
    keywords: ["drama", "დრამა", "family", "inspiration"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/81/Poster-pursuithappyness.jpg",
    trailer: "89Kq8SDyvfg",
  },
  {
    id: "dead-poets-society",
    title: "Dead Poets Society",
    year: 1989,
    rating: 8.1,
    duration: "2სთ 8წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Dead Poets Society.",
    keywords: ["drama", "დრამა", "inspiration"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/86/Dead_poets_society.png",
    trailer: "2Mn5UChANqI",
  },
  {
    id: "good-will-hunting",
    title: "Good Will Hunting",
    year: 1997,
    rating: 8.3,
    duration: "2სთ 6წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Good Will Hunting.",
    keywords: ["drama", "დრამა", "inspiration"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/52/Good_Will_Hunting.png",
    trailer: "PaZVjZEFkRs",
  },
  {
    id: "million-dollar-baby",
    title: "Million Dollar Baby",
    year: 2004,
    rating: 8.1,
    duration: "2სთ 12წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Million Dollar Baby.",
    keywords: ["drama", "დრამა", "sport", "emotional"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d3/Million_Dollar_Baby_poster.jpg",
    trailer: "GNMwuptdM8Q",
  },
  {
    id: "the-wolf-of-wall-street",
    title: "The Wolf of Wall Street",
    year: 2013,
    rating: 8.2,
    duration: "3სთ 0წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — The Wolf of Wall Street.",
    keywords: ["drama", "დრამა", "crime", "comedy"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d8/The_Wolf_of_Wall_Street_%282013%29.png",
    trailer: "iszwuX1AK6A",
  },
  {
    id: "nomadland",
    title: "Nomadland",
    year: 2020,
    rating: 7.3,
    duration: "1სთ 47წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Nomadland.",
    keywords: ["drama", "დრამა", "life"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/a5/Nomadland_poster.jpeg",
    trailer: "6sxCFZ8_d84",
  },
  {
    id: "manchester-by-the-sea",
    title: "Manchester by the Sea",
    year: 2016,
    rating: 7.8,
    duration: "2სთ 17წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Manchester by the Sea.",
    keywords: ["drama", "დრამა", "loss", "grief", "lost", "დაკარგვა"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/de/Manchester_by_the_Sea.jpg",
    trailer: "gy_MTfaZQ-c",
  },
  {
    id: "prisoners",
    title: "Prisoners",
    year: 2013,
    rating: 8.2,
    duration: "2სთ 33წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Prisoners.",
    keywords: ["drama", "დრამა", "thriller", "mystery"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/63/Prisoners2013Poster.jpg",
    trailer: "bpEIvUxLYlQ",
  },
  {
    id: "shutter-island",
    title: "Shutter Island",
    year: 2010,
    rating: 8.2,
    duration: "2სთ 18წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Shutter Island.",
    keywords: ["drama", "დრამა", "thriller", "mystery"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/76/Shutterislandposter.jpg",
    trailer: "5iaYLCiq5RM",
  },
  {
    id: "gone-girl",
    title: "Gone Girl",
    year: 2014,
    rating: 8.1,
    duration: "2სთ 29წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Gone Girl.",
    keywords: ["drama", "დრამა", "thriller", "mystery"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/05/Gone_Girl_Poster.jpg",
    trailer: "2-_-1nJf8Vg",
  },
  {
    id: "no-country-for-old-men",
    title: "No Country for Old Men",
    year: 2007,
    rating: 8.2,
    duration: "2სთ 2წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — No Country for Old Men.",
    keywords: ["drama", "დრამა", "crime", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8b/No_Country_for_Old_Men_poster.jpg",
    trailer: "38A__WT3-o0",
  },
  {
    id: "there-will-be-blood",
    title: "There Will Be Blood",
    year: 2007,
    rating: 8.2,
    duration: "2სთ 38წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — There Will Be Blood.",
    keywords: ["drama", "დრამა", "history"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/da/There_Will_Be_Blood_Poster.jpg",
    trailer: "3vGVWTfaRRc",
  },
  {
    id: "oppenheimer",
    title: "Oppenheimer",
    year: 2023,
    rating: 8.3,
    duration: "3სთ 0წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Oppenheimer.",
    keywords: ["drama", "დრამა", "history", "war", "ომი", "biography"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/4a/Oppenheimer_%28film%29.jpg",
    trailer: "uYPbbksJxIg",
  },
  {
    id: "the-social-network",
    title: "The Social Network",
    year: 2010,
    rating: 7.8,
    duration: "2სთ 0წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — The Social Network.",
    keywords: ["drama", "დრამა", "biography", "tech"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8c/The_Social_Network_film_poster.png",
    trailer: "lB95KLmpLR4",
  },
  {
    id: "slumdog-millionaire",
    title: "Slumdog Millionaire",
    year: 2008,
    rating: 8.0,
    duration: "2სთ 0წთ",
    category: "drama",
    description:
      "ღრმა, ემოციური დრამა ადამიანურ ბედზე, არჩევანსა და ცხოვრებაზე. — Slumdog Millionaire.",
    keywords: ["drama", "დრამა", "love", "სიყვარული"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/92/Slumdog_Millionaire_poster.png",
    trailer: "AIzbwV7on6Q",
  },
  {
    id: "hereditary",
    title: "Hereditary",
    year: 2018,
    rating: 7.3,
    duration: "2სთ 7წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Hereditary.",
    keywords: ["horror", "საშიში", "scary", "thriller", "family"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d9/Hereditary.png",
    trailer: "V6wWKNij_1M",
  },
  {
    id: "the-exorcist",
    title: "The Exorcist",
    year: 1973,
    rating: 8.1,
    duration: "2სთ 2წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — The Exorcist.",
    keywords: ["horror", "საშიში", "scary", "supernatural", "classic"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/7b/Exorcist_ver2.jpg",
    trailer: "YDGw1MTEe9k",
  },
  {
    id: "midsommar",
    title: "Midsommar",
    year: 2019,
    rating: 7.1,
    duration: "2სთ 28წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Midsommar.",
    keywords: ["horror", "საშიში", "scary", "thriller", "cult"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/47/Midsommar_%282019_film_poster%29.png",
    trailer: "1Vnghdsjmd0",
  },
  {
    id: "us",
    title: "Us",
    year: 2019,
    rating: 6.8,
    duration: "1სთ 56წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Us.",
    keywords: ["horror", "საშიში", "scary", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/00/Us_%282019%29_theatrical_poster.png",
    trailer: "hNCmb-4oXJA",
  },
  {
    id: "the-babadook",
    title: "The Babadook",
    year: 2014,
    rating: 6.8,
    duration: "1სთ 34წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — The Babadook.",
    keywords: ["horror", "საშიში", "scary", "thriller", "psychological"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d7/The-Babadook-Poster.jpg",
    trailer: "k5WQZzHmMwc",
  },
  {
    id: "insidious",
    title: "Insidious",
    year: 2010,
    rating: 6.8,
    duration: "1სთ 43წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Insidious.",
    keywords: ["horror", "საშიში", "scary", "supernatural"],
    image: "https://upload.wikimedia.org/wikipedia/en/2/2d/Insidious_poster.jpg",
    trailer: "zuZnRUcoWos",
  },
  {
    id: "sinister",
    title: "Sinister",
    year: 2012,
    rating: 6.8,
    duration: "1სთ 50წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Sinister.",
    keywords: ["horror", "საშიში", "scary", "supernatural"],
    image: "https://upload.wikimedia.org/wikipedia/en/f/f8/SinisterMoviePoster2012.jpg",
    trailer: "_kkGO_c9E1Q",
  },
  {
    id: "the-witch",
    title: "The Witch",
    year: 2015,
    rating: 6.9,
    duration: "1სთ 32წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — The Witch.",
    keywords: ["horror", "საშიში", "scary", "thriller", "folk"],
    image: "https://upload.wikimedia.org/wikipedia/en/b/bf/The_Witch_poster.png",
    trailer: "iQXmlf3Sefg",
  },
  {
    id: "nosferatu",
    title: "Nosferatu",
    year: 2024,
    rating: 7.1,
    duration: "2სთ 12წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Nosferatu.",
    keywords: ["horror", "საშიში", "scary", "vampire", "gothic"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/48/Nosferatu_IMAX_poster_2024.jpg",
    trailer: "XZBWq2Jm9WY",
  },
  {
    id: "halloween",
    title: "Halloween",
    year: 1978,
    rating: 7.7,
    duration: "1სთ 31წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Halloween.",
    keywords: ["horror", "საშიში", "scary", "slasher", "classic"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/af/Halloween_%281978%29_theatrical_poster.jpg",
    trailer: "",
  },
  {
    id: "scream",
    title: "Scream",
    year: 1996,
    rating: 7.4,
    duration: "1სთ 51წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Scream.",
    keywords: ["horror", "საშიში", "scary", "slasher", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/86/Scream_%281996_film%29_poster.jpg",
    trailer: "tAOwFbS_6Bg",
  },
  {
    id: "the-ring",
    title: "The Ring",
    year: 2002,
    rating: 7.1,
    duration: "1სთ 55წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — The Ring.",
    keywords: ["horror", "საშიში", "scary", "supernatural", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/37/Theringpostere.jpg",
    trailer: "1lS5xWU9_Nc",
  },
  {
    id: "smile",
    title: "Smile",
    year: 2022,
    rating: 6.5,
    duration: "1სთ 55წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — Smile.",
    keywords: ["horror", "საშიში", "scary", "thriller", "psychological"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/7f/Smile_%282022_film%29.jpg",
    trailer: "hFj4dj2mtdA",
  },
  {
    id: "the-nun",
    title: "The Nun",
    year: 2018,
    rating: 5.3,
    duration: "1სთ 36წთ",
    category: "horror",
    description:
      "დამძაბავი საშინელებათა ფილმი, რომელიც სუნთქვას შეგიკრავს. — The Nun.",
    keywords: ["horror", "საშიში", "scary", "supernatural"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/34/TheNunPoster.jpg",
    trailer: "pzD9zGcUNrw",
  },
  {
    id: "call-me-by-your-name",
    title: "Call Me by Your Name",
    year: 2017,
    rating: 7.8,
    duration: "2სთ 12წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — Call Me by Your Name.",
    keywords: ["love", "romance", "სიყვარული", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/c/c9/CallMeByYourName2017.png",
    trailer: "Z9AYPxH5NTM",
  },
  {
    id: "500-days-of-summer",
    title: "500 Days of Summer",
    year: 2009,
    rating: 7.7,
    duration: "1სთ 35წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — 500 Days of Summer.",
    keywords: ["love", "romance", "სიყვარული", "comedy", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d1/Five_hundred_days_of_summer.jpg",
    trailer: "PsD0NpFSADM",
  },
  {
    id: "before-sunrise",
    title: "Before Sunrise",
    year: 1995,
    rating: 8.1,
    duration: "1სთ 41წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — Before Sunrise.",
    keywords: ["love", "romance", "სიყვარული", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/da/Before_Sunrise_poster.jpg",
    trailer: "Tp7xntWA8Zk",
  },
  {
    id: "me-before-you",
    title: "Me Before You",
    year: 2016,
    rating: 7.4,
    duration: "1სთ 46წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — Me Before You.",
    keywords: ["love", "romance", "სიყვარული", "drama", "emotional"],
    image: "https://upload.wikimedia.org/wikipedia/en/f/fd/Me_Before_You_%28film%29.jpg",
    trailer: "Eh993__rOxA",
  },
  {
    id: "a-star-is-born",
    title: "A Star Is Born",
    year: 2018,
    rating: 7.6,
    duration: "2სთ 16წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — A Star Is Born.",
    keywords: ["love", "romance", "სიყვარული", "music", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/39/A_Star_is_Born.png",
    trailer: "nSbzyEJ8X9E",
  },
  {
    id: "the-fault-in-our-stars",
    title: "The Fault in Our Stars",
    year: 2014,
    rating: 7.7,
    duration: "2სთ 6წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — The Fault in Our Stars.",
    keywords: ["love", "romance", "სიყვარული", "drama", "emotional"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/41/The_Fault_in_Our_Stars_%28Official_Film_Poster%29.png",
    trailer: "H0Y6bqjIQvo",
  },
  {
    id: "ghost",
    title: "Ghost",
    year: 1990,
    rating: 7.1,
    duration: "2სთ 7წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — Ghost.",
    keywords: ["love", "romance", "სიყვარული", "supernatural", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/41/Ghost_%281990_movie_poster%29.jpg",
    trailer: "GVAgg_lPFVc",
  },
  {
    id: "blue-valentine",
    title: "Blue Valentine",
    year: 2010,
    rating: 7.4,
    duration: "1სთ 52წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — Blue Valentine.",
    keywords: ["love", "romance", "სიყვარული", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/04/Blue_Valentine_film.jpg",
    trailer: "gzWDJs5-vBw",
  },
  {
    id: "brooklyn",
    title: "Brooklyn",
    year: 2015,
    rating: 7.5,
    duration: "1სთ 51წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — Brooklyn.",
    keywords: ["love", "romance", "სიყვარული", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/5b/Brooklyn_FilmPoster.jpg",
    trailer: "15syDwC000k",
  },
  {
    id: "about-time",
    title: "About Time",
    year: 2013,
    rating: 7.8,
    duration: "2სთ 3წთ",
    category: "romance",
    description:
      "გულის ამაჩუყებელი სასიყვარულო ისტორია, სადაც გრძნობები ბედს ეჯახება. — About Time.",
    keywords: ["love", "romance", "სიყვარული", "comedy", "time"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/7c/About_Time_%282013_film%29_Poster.jpg",
    trailer: "MvGXqjS-Uxg",
  },
  {
    id: "inside-out",
    title: "Inside Out",
    year: 2015,
    rating: 8.1,
    duration: "1სთ 35წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Inside Out.",
    keywords: ["animation", "მულტფილმი", "family", "emotion"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/0a/Inside_Out_%282015_film%29_poster.jpg",
    trailer: "",
  },
  {
    id: "ratatouille",
    title: "Ratatouille",
    year: 2007,
    rating: 8.1,
    duration: "1სთ 51წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Ratatouille.",
    keywords: ["animation", "მულტფილმი", "family", "comedy"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/50/RatatouillePoster.jpg",
    trailer: "c3sBBRxDAqk",
  },
  {
    id: "how-to-train-your-dragon",
    title: "How to Train Your Dragon",
    year: 2010,
    rating: 8.1,
    duration: "1სთ 38წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — How to Train Your Dragon.",
    keywords: ["animation", "მულტფილმი", "family", "adventure"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/99/How_to_Train_Your_Dragon_Poster.jpg",
    trailer: "2AsBksz0oT4",
  },
  {
    id: "kung-fu-panda",
    title: "Kung Fu Panda",
    year: 2008,
    rating: 7.6,
    duration: "1სთ 32წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Kung Fu Panda.",
    keywords: ["animation", "მულტფილმი", "family", "comedy", "action"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/76/Kungfupanda.jpg",
    trailer: "10Q8dhSHOGA",
  },
  {
    id: "zootopia",
    title: "Zootopia",
    year: 2016,
    rating: 8.0,
    duration: "1სთ 48წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Zootopia.",
    keywords: ["animation", "მულტფილმი", "family", "comedy"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/96/Zootopia_%28movie_poster%29.jpg",
    trailer: "jWM0ct-OLsM",
  },
  {
    id: "moana",
    title: "Moana",
    year: 2016,
    rating: 7.6,
    duration: "1სთ 47წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Moana.",
    keywords: ["animation", "მულტფილმი", "family", "musical", "adventure"],
    image: "https://upload.wikimedia.org/wikipedia/en/2/26/Moana_Teaser_Poster.jpg",
    trailer: "LKFuXETZUsI",
  },
  {
    id: "despicable-me",
    title: "Despicable Me",
    year: 2010,
    rating: 7.6,
    duration: "1სთ 35წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Despicable Me.",
    keywords: ["animation", "მულტფილმი", "family", "comedy"],
    image: "https://upload.wikimedia.org/wikipedia/en/c/c0/Despicable_Me_%282010_animated_feature_film%29.jpg",
    trailer: "sUkZFepDljo",
  },
  {
    id: "the-incredibles",
    title: "The Incredibles",
    year: 2004,
    rating: 8.0,
    duration: "1სთ 55წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — The Incredibles.",
    keywords: ["animation", "მულტფილმი", "family", "action", "superhero"],
    image: "https://upload.wikimedia.org/wikipedia/en/2/27/The_Incredibles_%282004_animated_feature_film%29.jpg",
    trailer: "-UaGUdNJdRQ",
  },
  {
    id: "monsters-inc",
    title: "Monsters, Inc.",
    year: 2001,
    rating: 8.1,
    duration: "1სთ 32წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Monsters, Inc..",
    keywords: ["animation", "მულტფილმი", "family", "comedy"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/63/Monsters_Inc.JPG",
    trailer: "kN5eD5tzbkc",
  },
  {
    id: "your-name",
    title: "Your Name",
    year: 2016,
    rating: 8.4,
    duration: "1სთ 46წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Your Name.",
    keywords: ["animation", "მულტფილმი", "family", "anime", "love", "romance", "სიყვარული"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/0b/Your_Name_poster.png",
    trailer: "xU47nhruN-Q",
  },
  {
    id: "princess-mononoke",
    title: "Princess Mononoke",
    year: 1997,
    rating: 8.4,
    duration: "2სთ 14წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Princess Mononoke.",
    keywords: ["animation", "მულტფილმი", "family", "anime", "adventure"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8c/Princess_Mononoke_Japanese_poster.png",
    trailer: "4OiMOHRDs14",
  },
  {
    id: "howls-moving-castle",
    title: "Howl's Moving Castle",
    year: 2004,
    rating: 8.2,
    duration: "1სთ 59წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Howl's Moving Castle.",
    keywords: ["animation", "მულტფილმი", "family", "anime", "fantasy"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/a0/Howls-moving-castleposter.jpg",
    trailer: "iwROgK94zcM",
  },
  {
    id: "encanto",
    title: "Encanto",
    year: 2021,
    rating: 7.2,
    duration: "1სთ 42წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Encanto.",
    keywords: ["animation", "მულტფილმი", "family", "musical"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/83/Encanto_poster.jpg",
    trailer: "CaimKeDcudo",
  },
  {
    id: "soul",
    title: "Soul",
    year: 2020,
    rating: 8.0,
    duration: "1სთ 40წთ",
    category: "animation",
    description:
      "ფერადი და თბილი სათავგადასავლო ანიმაცია, რომელიც მთელ ოჯახს მოხიბლავს. — Soul.",
    keywords: ["animation", "მულტფილმი", "family", "music"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/39/Soul_%282020_film%29_poster.jpg",
    trailer: "xOsLIiBStEs",
  },
  {
    id: "the-hangover",
    title: "The Hangover",
    year: 2009,
    rating: 7.7,
    duration: "1სთ 40წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — The Hangover.",
    keywords: ["comedy", "კომედია", "funny", "adventure"],
    image: "https://upload.wikimedia.org/wikipedia/en/b/b9/Hangoverposter09.jpg",
    trailer: "tcdUhdOlz9M",
  },
  {
    id: "superbad",
    title: "Superbad",
    year: 2007,
    rating: 7.6,
    duration: "1სთ 53წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — Superbad.",
    keywords: ["comedy", "კომედია", "funny", "teen"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8b/Superbad_Poster.png",
    trailer: "4eaZ_48ZYog",
  },
  {
    id: "21-jump-street",
    title: "21 Jump Street",
    year: 2012,
    rating: 7.2,
    duration: "1სთ 49წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — 21 Jump Street.",
    keywords: ["comedy", "კომედია", "funny", "action"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/93/21JumpStreetfilm.jpg",
    trailer: "RZ-Gs-72Jyw",
  },
  {
    id: "dumb-and-dumber",
    title: "Dumb and Dumber",
    year: 1994,
    rating: 7.3,
    duration: "1სთ 47წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — Dumb and Dumber.",
    keywords: ["comedy", "კომედია", "funny", "classic"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/64/Dumbanddumber.jpg",
    trailer: "l13yPhimE3o",
  },
  {
    id: "bridesmaids",
    title: "Bridesmaids",
    year: 2011,
    rating: 6.8,
    duration: "2სთ 5წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — Bridesmaids.",
    keywords: ["comedy", "კომედია", "funny", "friendship"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/df/BridesmaidsPoster.jpg",
    trailer: "Ba2_Q8pl9UI",
  },
  {
    id: "borat",
    title: "Borat",
    year: 2006,
    rating: 7.3,
    duration: "1სთ 24წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — Borat.",
    keywords: ["comedy", "კომედია", "funny", "satire"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/39/Borat_ver2.jpg",
    trailer: "Jgi8y1KGrGw",
  },
  {
    id: "groundhog-day",
    title: "Groundhog Day",
    year: 1993,
    rating: 8.0,
    duration: "1სთ 41წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — Groundhog Day.",
    keywords: ["comedy", "კომედია", "funny", "time", "classic"],
    image: "https://upload.wikimedia.org/wikipedia/en/b/b1/Groundhog_Day_%28movie_poster%29.jpg",
    trailer: "tSVeDx9fk60",
  },
  {
    id: "deadpool",
    title: "Deadpool",
    year: 2016,
    rating: 8.0,
    duration: "1სთ 48წთ",
    category: "comedy",
    description:
      "მხიარული და მსუბუქი კომედია, რომელიც განწყობას აგიმაღლებს. — Deadpool.",
    keywords: ["comedy", "კომედია", "funny", "action", "superhero"],
    image: "https://upload.wikimedia.org/wikipedia/en/2/23/Deadpool_%282016_poster%29.png",
    trailer: "ONHBaC-pfsk",
  },
  {
    id: "dunkirk",
    title: "Dunkirk",
    year: 2017,
    rating: 7.8,
    duration: "1სთ 46წთ",
    category: "war",
    description:
      "ომის სისასტიკესა და ადამიანურ სიმამაცეზე მოთხრობილი ძლიერი ისტორია. — Dunkirk.",
    keywords: ["war", "ომი", "history", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/1/15/Dunkirk_Film_poster.jpg",
    trailer: "F-eMt3SrfFU",
  },
  {
    id: "hacksaw-ridge",
    title: "Hacksaw Ridge",
    year: 2016,
    rating: 8.1,
    duration: "2სთ 19წთ",
    category: "war",
    description:
      "ომის სისასტიკესა და ადამიანურ სიმამაცეზე მოთხრობილი ძლიერი ისტორია. — Hacksaw Ridge.",
    keywords: ["war", "ომი", "history", "biography"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/8a/Hacksaw_Ridge_poster.png",
    trailer: "s2-1hz1juBI",
  },
  {
    id: "apocalypse-now",
    title: "Apocalypse Now",
    year: 1979,
    rating: 8.4,
    duration: "2სთ 27წთ",
    category: "war",
    description:
      "ომის სისასტიკესა და ადამიანურ სიმამაცეზე მოთხრობილი ძლიერი ისტორია. — Apocalypse Now.",
    keywords: ["war", "ომი", "history", "classic", "epic"],
    image: "https://upload.wikimedia.org/wikipedia/en/c/c2/Apocalypse_Now_poster.jpg",
    trailer: "FTjG-Aux_yQ",
  },
  {
    id: "full-metal-jacket",
    title: "Full Metal Jacket",
    year: 1987,
    rating: 8.3,
    duration: "1სთ 56წთ",
    category: "war",
    description:
      "ომის სისასტიკესა და ადამიანურ სიმამაცეზე მოთხრობილი ძლიერი ისტორია. — Full Metal Jacket.",
    keywords: ["war", "ომი", "history", "classic"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/99/Full_Metal_Jacket_poster.jpg",
    trailer: "Lv2mJKcHNbA",
  },
  {
    id: "platoon",
    title: "Platoon",
    year: 1986,
    rating: 8.1,
    duration: "2სთ 0წთ",
    category: "war",
    description:
      "ომის სისასტიკესა და ადამიანურ სიმამაცეზე მოთხრობილი ძლიერი ისტორია. — Platoon.",
    keywords: ["war", "ომი", "history", "classic"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/a9/Platoon_posters_86.jpg",
    trailer: "H-5Ff3sTUcU",
  },
  {
    id: "the-pianist",
    title: "The Pianist",
    year: 2002,
    rating: 8.5,
    duration: "2სთ 30წთ",
    category: "war",
    description:
      "ომის სისასტიკესა და ადამიანურ სიმამაცეზე მოთხრობილი ძლიერი ისტორია. — The Pianist.",
    keywords: ["war", "ომი", "history", "biography", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/a6/The_Pianist_movie.jpg",
    trailer: "fPFxUeK_bpc",
  },

  // ─────────────── ახალი დამატებული ფილმები ───────────────
  {
    id: "the-prestige",
    title: "The Prestige",
    year: 2006,
    rating: 8.5,
    duration: "2სთ 10წთ",
    category: "sci-fi",
    description:
      "ორი ილუზიონისტი სასიკვდილო მეტოქეობაში ერთვება, სადაც საიდუმლოს გასაღები საკუთარ თავზეც კი ძვირად ჯდება.",
    keywords: ["magic", "საიდუმლო", "mystery", "rivalry", "sci-fi", "ფანტასტიკა", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d2/Prestige_poster.jpg",
    trailer: "RLtaA9fFNXU",
  },
  {
    id: "children-of-men",
    title: "Children of Men",
    year: 2006,
    rating: 7.9,
    duration: "1სთ 49წთ",
    category: "sci-fi",
    description:
      "მსოფლიოში, სადაც ბავშვები აღარ იბადებიან, ერთი კაცი უნდა დაიცვას უკანასკნელი ორსული ქალი კაცობრიობის იმედად.",
    keywords: ["dystopia", "მომავალი", "future", "survival", "sci-fi", "ფანტასტიკა"],
    image: "https://upload.wikimedia.org/wikipedia/en/f/fc/Children_of_men_ver4.jpg",
    trailer: "2VT2apoX90o",
  },
  {
    id: "moon",
    title: "Moon",
    year: 2009,
    rating: 7.8,
    duration: "1სთ 37წთ",
    category: "sci-fi",
    description:
      "მთვარეზე მარტოსული მუშა კონტრაქტის დასრულებამდე საკუთარი არსებობის შემაძრწუნებელ სიმართლეს აღმოაჩენს.",
    keywords: ["space", "კოსმოსი", "isolation", "clone", "sci-fi", "ფანტასტიკა"],
    image: "https://upload.wikimedia.org/wikipedia/en/a/af/Moon_%282009_film%29.jpg",
    trailer: "WWoDBcSW4_c",
  },
  {
    id: "the-dark-knight-rises",
    title: "The Dark Knight Rises",
    year: 2012,
    rating: 8.4,
    duration: "2სთ 44წთ",
    category: "action",
    description:
      "რვა წლის შემდეგ ბეტმენი ბრუნდება, რათა გოთემი გადაარჩინოს ბოროტი ბეინის ტერორისგან.",
    keywords: ["batman", "გმირი", "hero", "dc", "action", "ექშენი"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/83/Dark_knight_rises_poster.jpg",
    trailer: "GokKUqLcvD8",
  },
  {
    id: "heat",
    title: "Heat",
    year: 1995,
    rating: 8.3,
    duration: "2სთ 50წთ",
    category: "action",
    description:
      "გამოცდილი დეტექტივი და დახვეწილი ყაჩაღი ბედისწერულ დაპირისპირებაში ხვდებიან ლოს-ანჯელესის ქუჩებში.",
    keywords: ["heist", "ქურდობა", "crime", "police", "action", "ექშენი"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/6c/Heatposter.jpg",
    trailer: "2GfZl4kuVNI",
  },
  {
    id: "wonder-woman",
    title: "Wonder Woman",
    year: 2017,
    rating: 7.4,
    duration: "2სთ 21წთ",
    category: "action",
    description:
      "ამორძალი დიანა ტოვებს კუნძულს, რათა ომში ჩაერთოს და კაცობრიობა ბოროტებისგან დაიცვას.",
    keywords: ["hero", "გმირი", "dc", "warrior", "action", "ექშენი"],
    image: "https://upload.wikimedia.org/wikipedia/en/b/b0/Wonder_Woman_%282017_film%29_poster.jpg",
    trailer: "1Q8fG0TtVAY",
  },
  {
    id: "captain-america-civil-war",
    title: "Captain America: Civil War",
    year: 2016,
    rating: 7.8,
    duration: "2სთ 27წთ",
    category: "action",
    description:
      "შურისმაძიებლები ორ ბანაკად იყოფიან, როცა კონტროლის საკითხი მათ შორის ჭეშმარიტ ომს იწვევს.",
    keywords: ["marvel", "გმირი", "hero", "avengers", "action", "ექშენი"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/53/Captain_America_Civil_War_poster.jpg",
    trailer: "dKrVegVI0Us",
  },
  {
    id: "american-beauty",
    title: "American Beauty",
    year: 1999,
    rating: 8.3,
    duration: "2სთ 2წთ",
    category: "drama",
    description:
      "საშუალო ასაკის კაცი ცხოვრებისეულ კრიზისში ცდილობს თავიდან იპოვოს აზრი და თავისუფლება.",
    keywords: ["life", "ცხოვრება", "family", "crisis", "drama", "დრამა"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/9a/American_Beauty_1999_film_poster.jpg",
    trailer: "XCxzXblZyfQ",
  },
  {
    id: "requiem-for-a-dream",
    title: "Requiem for a Dream",
    year: 2000,
    rating: 8.3,
    duration: "1სთ 42წთ",
    category: "drama",
    description:
      "ოთხი ადამიანის ოცნებები ნელ-ნელა ინგრევა დამოკიდებულების უფსკრულში.",
    keywords: ["addiction", "ტკივილი", "tragedy", "dream", "drama", "დრამა"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/92/Requiem_for_a_dream.jpg",
    trailer: "0nU7dC9bIDg",
  },
  {
    id: "into-the-wild",
    title: "Into the Wild",
    year: 2007,
    rating: 8.1,
    duration: "2სთ 28წთ",
    category: "drama",
    description:
      "ახალგაზრდა კაცი ტოვებს ყველაფერს და ველურ ბუნებაში ეძებს ცხოვრების ჭეშმარიტ აზრს.",
    keywords: ["adventure", "თავისუფლება", "freedom", "nature", "drama", "დრამა"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/dc/Into_the_Wild_%282007_film_poster%29.png",
    trailer: "XZG1FzyB8DI",
  },
  {
    id: "the-imitation-game",
    title: "The Imitation Game",
    year: 2014,
    rating: 8.0,
    duration: "1სთ 54წთ",
    category: "drama",
    description:
      "გენიოსი მათემატიკოსი ალან ტიურინგი მეორე მსოფლიო ომში ნაცისტების საიდუმლო კოდს ხსნის.",
    keywords: ["genius", "ისტორია", "war", "code", "drama", "დრამა"],
    image: "https://upload.wikimedia.org/wikipedia/en/8/87/The_Imitation_Game_%282014%29.png",
    trailer: "nuPZUUED5uk",
  },
  {
    id: "green-book",
    title: "Green Book",
    year: 2018,
    rating: 8.2,
    duration: "2სთ 10წთ",
    category: "drama",
    description:
      "შავკანიანი პიანისტი და მისი თეთრკანიანი მძღოლი ერთად მოგზაურობენ სამხრეთის რასიზმის ფონზე.",
    keywords: ["friendship", "მეგობრობა", "race", "journey", "drama", "დრამა"],
    image: "https://upload.wikimedia.org/wikipedia/en/5/5b/Green_Book_%282018_poster%29.png",
    trailer: "QkZxoko_HC0",
  },
  {
    id: "the-conjuring-2",
    title: "The Conjuring 2",
    year: 2016,
    rating: 7.3,
    duration: "2სთ 14წთ",
    category: "horror",
    description:
      "წყვილი პარანორმალური მკვლევრები ლონდონში ოჯახს ბოროტი სულისგან იცავენ.",
    keywords: ["ghost", "მოჩვენება", "demon", "horror", "საშიში", "paranormal"],
    image: "https://upload.wikimedia.org/wikipedia/en/2/27/The_Conjuring_2.jpg",
    trailer: "VFsmuRPClr4",
  },
  {
    id: "it-chapter-two",
    title: "It Chapter Two",
    year: 2019,
    rating: 6.5,
    duration: "2სთ 49წთ",
    category: "horror",
    description:
      "ოცდაშვიდი წლის შემდეგ მოზრდილი მეგობრები ბრუნდებიან, რომ ბოროტ პენივაისს დაუპირისპირდნენ.",
    keywords: ["clown", "შიში", "fear", "horror", "საშიში", "evil"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d8/It_Chapter_Two_Poster.jpg",
    trailer: "xhJ5P7Up3jA",
  },
  {
    id: "the-invisible-man",
    title: "The Invisible Man",
    year: 2020,
    rating: 7.1,
    duration: "2სთ 4წთ",
    category: "horror",
    description:
      "ქალი დარწმუნებულია, რომ მისი გარდაცვლილი ყოფილი პარტნიორი უხილავად მისდევს.",
    keywords: ["invisible", "ტერორი", "stalker", "horror", "საშიში", "thriller"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/3a/The_Invisible_Man_%282020_film%29_-_release_poster.jpg",
    trailer: "WO_FJdiY9dA",
  },
  {
    id: "28-days-later",
    title: "28 Days Later",
    year: 2002,
    rating: 7.5,
    duration: "1სთ 53წთ",
    category: "horror",
    description:
      "ვირუსის აფეთქების შემდეგ გადარჩენილები ცდილობენ გაექცნენ ინფიცირებულთა ურდოს.",
    keywords: ["zombie", "ვირუსი", "survival", "horror", "საშიში", "apocalypse"],
    image: "https://upload.wikimedia.org/wikipedia/en/e/e4/28_days_later.jpg",
    trailer: "mWEhfF27O0c",
  },
  {
    id: "silver-linings-playbook",
    title: "Silver Linings Playbook",
    year: 2012,
    rating: 7.7,
    duration: "2სთ 2წთ",
    category: "romance",
    description:
      "ორი დაზიანებული სული ცეკვის რიტმში პოულობს იმედსა და სიყვარულს.",
    keywords: ["love", "სიყვარული", "dance", "hope", "romance", "recovery"],
    image: "https://upload.wikimedia.org/wikipedia/en/9/9a/Silver_Linings_Playbook_Poster.jpg",
    trailer: "QJNdf_7pjb4",
  },
  {
    id: "crazy-stupid-love",
    title: "Crazy, Stupid, Love",
    year: 2011,
    rating: 7.4,
    duration: "1სთ 58წთ",
    category: "romance",
    description:
      "განქორწინებული კაცი სასიყვარულო ხელოვნებას სწავლობს, სანამ გულს ნამდვილ გრძნობას მიაგნებს.",
    keywords: ["love", "სიყვარული", "comedy", "relationship", "romance", "dating"],
    image: "https://upload.wikimedia.org/wikipedia/en/7/78/CrazyStupidLovePoster.jpg",
    trailer: "8iCwtxJejik",
  },
  {
    id: "the-vow",
    title: "The Vow",
    year: 2012,
    rating: 6.8,
    duration: "1სთ 44წთ",
    category: "romance",
    description:
      "ავარიის შემდეგ ქალი მეხსიერებას კარგავს და ქმარი ცდილობს მისი გულის ხელახლა მოგებას.",
    keywords: ["love", "სიყვარული", "memory", "marriage", "romance", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/c/c2/The_Vow_Poster.jpg",
    trailer: "PcL24s-S6ns",
  },
  {
    id: "toy-story-3",
    title: "Toy Story 3",
    year: 2010,
    rating: 8.3,
    duration: "1სთ 43წთ",
    category: "animation",
    description:
      "სათამაშოები ახალ თავგადასავალში ხვდებიან, როცა ენდი კოლეჯში მიდის.",
    keywords: ["toys", "მეგობრობა", "pixar", "animation", "მულტფილმები", "family"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/69/Toy_Story_3_poster.jpg",
    trailer: "2BlMNH1QTeE",
  },
  {
    id: "big-hero-6",
    title: "Big Hero 6",
    year: 2014,
    rating: 7.8,
    duration: "1სთ 42წთ",
    category: "animation",
    description:
      "ბიჭი და კეთილი რობოტი ბაიმაქსი გმირთა გუნდს ქმნიან ქალაქის გადასარჩენად.",
    keywords: ["robot", "რობოტი", "disney", "animation", "მულტფილმები", "hero"],
    image: "https://upload.wikimedia.org/wikipedia/en/4/4b/Big_Hero_6_%28film%29_poster.jpg",
    trailer: "z3biFxZIJOQ",
  },
  {
    id: "luca",
    title: "Luca",
    year: 2021,
    rating: 7.4,
    duration: "1სთ 35წთ",
    category: "animation",
    description:
      "ზღვის ურჩხული ბიჭი იტალიურ ქალაქში ზაფხულის დაუვიწყარ თავგადასავალს განიცდის.",
    keywords: ["summer", "ზღვა", "pixar", "friendship", "animation", "მულტფილმები"],
    image: "https://upload.wikimedia.org/wikipedia/en/3/33/Luca_%282021_film%29.png",
    trailer: "mYfJxlgR2jw",
  },
  {
    id: "klaus",
    title: "Klaus",
    year: 2019,
    rating: 8.1,
    duration: "1სთ 36წთ",
    category: "animation",
    description:
      "ეგოისტი ფოსტალიონი და იდუმალი სათამაშოების ოსტატი ერთად ნამდვილ საშობაო სასწაულს ქმნიან.",
    keywords: ["christmas", "შობა", "kindness", "animation", "მულტფილმები", "family"],
    image: "https://upload.wikimedia.org/wikipedia/en/2/26/Klaus_poster.jpeg",
    trailer: "taE3PwurhYM",
  },
  {
    id: "the-truman-show",
    title: "The Truman Show",
    year: 1998,
    rating: 8.2,
    duration: "1სთ 43წთ",
    category: "comedy",
    description:
      "კაცი აღმოაჩენს, რომ მთელი მისი ცხოვრება რეალითი-შოუა, რომელსაც მილიონები უყურებენ.",
    keywords: ["reality", "ცხოვრება", "satire", "comedy", "კომედია", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/c/cd/Trumanshow.jpg",
    trailer: "dlnmQbPGuls",
  },
  {
    id: "anchorman",
    title: "Anchorman",
    year: 2004,
    rating: 7.2,
    duration: "1სთ 34წთ",
    category: "comedy",
    description:
      "70-იანების ეგოისტი წამყვანი ბრძოლას იწყებს, როცა სტუდიაში ნიჭიერი ქალი ჩნდება.",
    keywords: ["news", "სიცილი", "funny", "comedy", "კომედია", "satire"],
    image: "https://upload.wikimedia.org/wikipedia/en/6/64/Movie_poster_Anchorman_The_Legend_of_Ron_Burgundy.jpg",
    trailer: "j3tKJoidT_o",
  },
  {
    id: "jumanji-welcome-to-the-jungle",
    title: "Jumanji: Welcome to the Jungle",
    year: 2017,
    rating: 6.9,
    duration: "1სთ 59წთ",
    category: "comedy",
    description:
      "ოთხი მოზარდი ვიდეო-თამაშში მოხვდება და გმირებად ქცეულებმა ჯუნგლები უნდა გაიარონ.",
    keywords: ["adventure", "თამაში", "game", "comedy", "კომედია", "jungle"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/dc/Jumanji_Welcome_to_the_Jungle.png",
    trailer: "2QKg5SZ_35I",
  },
  {
    id: "step-brothers",
    title: "Step Brothers",
    year: 2008,
    rating: 6.9,
    duration: "1სთ 38წთ",
    category: "comedy",
    description:
      "ორი ზრდასრული უსაქმური კაცი გერ-ძმებად იქცევა და ქაოსს ტოვებს ყველგან.",
    keywords: ["funny", "სიცილი", "brothers", "comedy", "კომედია", "family"],
    image: "https://upload.wikimedia.org/wikipedia/en/d/d9/StepbrothersMP08.jpg",
    trailer: "CewglxElBK0",
  },
  {
    id: "american-sniper",
    title: "American Sniper",
    year: 2014,
    rating: 7.3,
    duration: "2სთ 13წთ",
    category: "war",
    description:
      "საუკეთესო ამერიკელი სნაიპერის ცხოვრება ომსა და ოჯახს შორის ტკივილიან ბალანსშია.",
    keywords: ["war", "ომი", "sniper", "soldier", "military", "drama"],
    image: "https://upload.wikimedia.org/wikipedia/en/1/10/American_Sniper_poster.jpg",
    trailer: "99k3u9ay1gs",
  },
  {
    id: "fury",
    title: "Fury",
    year: 2014,
    rating: 7.5,
    duration: "2სთ 14წთ",
    category: "war",
    description:
      "ტანკის ეკიპაჟი მეორე მსოფლიო ომის დასასრულს გერმანიის სიღრმეში სასიკვდილო მისიას ასრულებს.",
    keywords: ["war", "ომი", "tank", "soldier", "military", "battle"],
    image: "https://upload.wikimedia.org/wikipedia/en/1/17/Fury_2014_poster.jpg",
    trailer: "q94n3eWOWXM",
  },
  {
    id: "black-hawk-down",
    title: "Black Hawk Down",
    year: 2001,
    rating: 7.7,
    duration: "2სთ 24წთ",
    category: "war",
    description:
      "ამერიკელი ჯარისკაცები სომალიში მისიის ჩავარდნის შემდეგ სასტიკ ბრძოლაში ებმებიან.",
    keywords: ["war", "ომი", "soldier", "battle", "military", "survival"],
    image: "https://upload.wikimedia.org/wikipedia/en/0/0c/Black_hawk_down_ver1.jpg",
    trailer: "2GfBkC3qs78",
  },
];

export const getMovieById = (id: string) =>
  movies.find((movie) => movie.id === id);

export const getMoviesByCategory = (key: CategoryKey) =>
  movies.filter((movie) => movie.category === key);
